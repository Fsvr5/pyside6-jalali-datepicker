...
def test_qdate_roundtrip(qtbot):
    ...
    assert widget.qdate_from_jalali(*widget.jalali_from_qdate(source)) == source


def test_picker_clear_and_restore(qtbot):
    picker = JalaliDatePicker(date=QDate(2026, 8, 29), clearable=True); qtbot.addWidget(picker)
    picker.clear()
    assert not picker.date().isValid() and picker.jalali_date() is None and picker.jalali_text() == ""
    picker.set_jalali_date(1405, 6, 7)
    assert picker.date().isValid() and picker.jalali_date() == (1405, 6, 7)


def test_picker_applies_theme(qtbot):
    picker = JalaliDatePicker(theme=Theme.DARK); qtbot.addWidget(picker)
    assert picker.styleSheet(); picker.apply_theme(Theme.LIGHT); assert picker.styleSheet()


def test_picker_date_range_validation(qtbot):
    picker = JalaliDatePicker(); qtbot.addWidget(picker)
    minimum, maximum = QDate(2026, 1, 1), QDate(2026, 12, 31)
    picker.set_date_range(minimum, maximum)
    assert picker.date_edit.minimumDate() == minimum and picker.date_edit.maximumDate() == maximum


def test_custom_calendar_jalali_navigation(qtbot):
    calendar = JalaliCalendarWidget(date=QDate(2024, 3, 20)); qtbot.addWidget(calendar)
    assert calendar.jalali_date() == (1403, 1, 1)
    calendar.next_month(); assert calendar.month_combo.currentIndex() == 1
    calendar.previous_month(); assert calendar.month_combo.currentIndex() == 0


def test_custom_calendar_set_jalali_date(qtbot):
    calendar = JalaliCalendarWidget(); qtbot.addWidget(calendar)
...
def test_calendar_custom_marker(qtbot):
    ...
    calendar.set_marker(date, DayMarker("موعد تولید", foreground="#2563EB", bold=True))
    fmt = calendar.calendar.dateTextFormat(date)
    assert fmt.toolTip() == "موعد تولید" and fmt.fontWeight() == 700
...
def test_calendar_friday_highlight_toggle(qtbot):
    ...
    fmt = calendar.calendar.weekdayTextFormat(Qt.DayOfWeek.Friday)
    assert fmt.foreground().style() == Qt.BrushStyle.NoBrush


def test_popup_picker_syncs_selected_date(qtbot):
    picker = JalaliPopupDatePicker(date=QDate(2024, 3, 20)); qtbot.addWidget(picker)
    selected = QDate(2024, 4, 1); picker._calendar_selected(selected)
    assert picker.date() == selected and picker.calendar.date() == selected


def test_popup_picker_range_propagates(qtbot):
    picker = JalaliPopupDatePicker(); qtbot.addWidget(picker)
    minimum, maximum = QDate(2026, 1, 1), QDate(2026, 12, 31)
    picker.set_date_range(minimum, maximum)
...
def test_range_rejects_reverse_dates(qtbot):
    try:
        widget = JalaliDateRangeEdit(start_date=QDate(2026, 8, 30), end_date=QDate(2026, 8, 29)); qtbot.addWidget(widget)
    except ValueError: pass
    else: raise AssertionError("Expected ValueError for reversed date range")


def test_range_keeps_end_on_or_after_start(qtbot):
    widget = JalaliDateRangeEdit(start_date=QDate(2026, 8, 1), end_date=QDate(2026, 8, 10)); qtbot.addWidget(widget)
    widget.start.setDate(QDate(2026, 8, 8))
    start, end = widget.date_range()
    assert start == QDate(2026, 8, 8) and end >= start
